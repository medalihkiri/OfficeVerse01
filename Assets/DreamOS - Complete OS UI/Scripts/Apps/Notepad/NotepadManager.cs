...
using TMPro;
using System.Linq; // Add this for Find()
...
namespace Michsky.DreamOS
{
    public class NotepadManager : MonoBehaviour
    {
        ...
        public void CreateNote()
        {
            ...
            int newNoteIndex = libraryAsset.notes.Count - 1;
            ...
            if (saveCustomNotes && notepadStoring != null)
            {
                // The new note is the last one in the list
                notepadStoring.CreateNoteOnServer(libraryAsset.notes[libraryAsset.notes.Count - 1]);
            }
        }
        ...
        public void UpdateNote()
        {
            ...
            if (currentNoteIndex <= libraryAsset.notes.Count - 1 && libraryAsset.notes[currentNoteIndex].isDeleted == false)
            {
                ...
                if (saveCustomNotes && notepadStoring != null)
                {
                    notepadStoring.UpdateNoteOnServer(libraryAsset.notes[currentNoteIndex]);
                }
            }
        }

        public void CreateStoredNote(NotepadStoring.NoteData noteData)
        {
            // Add a new note item to the library
            NotepadLibrary.NoteItem newNote = new NotepadLibrary.NoteItem
            {
            ...
            itemButton.onClick.AddListener(delegate
            {
                OpenCustomNote(noteData.noteTitle, noteData.noteContent);
                ...
                deleteButton.onClick.AddListener(delegate
                {
                    // --- REPLACE THE ENTIRE INSIDE OF THIS DELEGATE ---
                    int noteIndexToDelete = go.transform.GetSiblingIndex();
                    NotepadLibrary.NoteItem noteToDelete = libraryAsset.notes[noteIndexToDelete];

                    // Call the server to delete it first
                    if (saveCustomNotes && notepadStoring != null && !string.IsNullOrEmpty(noteToDelete._id))
                    {
                        notepadStoring.DeleteNoteOnServer(noteToDelete._id);
                    }

                    // Now remove from the local list and update UI
                    libraryAsset.notes.RemoveAt(noteIndexToDelete);
                    go.SetActive(false);
                    viewerAnimator.Play("Out");
                    // We no longer need to call UpdateData() here.
                });
            });
